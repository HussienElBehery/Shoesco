...
export default async function AdminLoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#181b21] px-5">
      <div className="w-full max-w-md rounded-[2rem] bg-[#181b21] p-8 shadow-xl sm:p-10">
        <BrandMark />
        ...
        <form action={signIn} className="mt-7 space-y-5">
          ...
          <button className="w-full rounded-full bg-neutral-950 px-6 py-3.5 text-sm font-semibold text-[#f4f1ea]" type="submit">Sign in</button>
        </form>
      </div>
    </main>
  );
