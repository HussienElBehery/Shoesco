...
export function CartLink({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <Link
      ...
      className="relative flex h-10 w-10 items-center justify-center rounded-full border border-neutral-200 bg-white transition hover:bg-neutral-100"
      href="/cart"
    >
      ...
      {itemCount > 0 && (
        <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#b78955] px-1 text-[10px] font-bold text-white">
          {itemCount}
        </span>
      )}
    </Link>
