...
export function ProductGallery({
  ...
  return (
    ...
    >
      <div
        className={`relative overflow-hidden rounded-[1.75rem] border border-[#2a2e36] bg-[#f4f1ea] ${compact ? "aspect-[4/3]" : "aspect-square"}`}
        onTouchEnd={(event) => {
      ...
      >
        ...
        <Image
          ...
          className={`object-contain p-5 mix-blend-multiply transition duration-500 ${zoomed ? "scale-150" : "scale-100"}`}
          fill
          loading="eager"
          priority={!compact}
        ...
        <div className="grid grid-cols-5 gap-2" role="list" aria-label="Product images">
          {images.map((entry, index) => (
            <button
              ...
              className={`relative aspect-square overflow-hidden rounded-xl border bg-[#f4f1ea] ${index === activeIndex ? "border-[#c6ff3a]" : "border-[#2a2e36]"}`}
              key={entry.id}
              onClick={() => show(index)}
              type="button"
            >
